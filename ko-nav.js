...
// Crumbs updaten zonder rebuild: KONav.updateCrumbs('Evaluaties', [{ label: 'BAR 2' }, { label: 'Evaluatie toevoegen' }])
...
const KONav = {
  ...
  // ── Update alleen de broodkruimels, zonder navbar opnieuw te bouwen ──
  updateCrumbs(module, crumbs = []) {
    this._module = module;
    const nav = document.getElementById('ko-nav');
    if (!nav) return;

    // Verwijder bestaande divider + crumbs
    const bestaandeDivider = nav.querySelector('.ko-divider');
    const bestaandeCrumbs  = nav.querySelector('.ko-crumbs');
    if (bestaandeDivider) bestaandeDivider.remove();
    if (bestaandeCrumbs)  bestaandeCrumbs.remove();

    const allCrumbs = [];
    if (module) allCrumbs.push({ label: module, href: this._moduleHref(module) });
    crumbs.forEach(c => allCrumbs.push(c));

    if (allCrumbs.length === 0) return;
    ...
    // Voeg in vóór het user-blok
    const userEl = nav.querySelector('.ko-user');
    nav.insertBefore(divider, userEl);
    nav.insertBefore(crumbsEl, userEl);
  },
  ...
  _build(module, crumbs, accentColor) {
    ...
    if (!document.getElementById('ko-nav-style')) {
      ...
      style.textContent = `
        ...
        #ko-nav .ko-logo { display: flex; align-items: center; gap: 10px; text-decoration: none; flex-shrink: 0; }
        #ko-nav .ko-logo img { width: 32px; height: 32px; border-radius: 50%; object-fit: cover; }
        #ko-nav .ko-logo-txt { font-size: 15px; font-weight: 600; color: var(--ko-black); letter-spacing: -0.02em; }
        #ko-nav .ko-divider { width: 0.5px; height: 20px; background: var(--ko-border); margin: 0 14px; flex-shrink: 0; }
        #ko-nav .ko-crumbs { display: flex; align-items: center; gap: 0; flex: 1; min-width: 0; overflow: hidden; }
        #ko-nav .ko-crumb { display: flex; align-items: center; gap: 0; white-space: nowrap; min-width: 0; }
        #ko-nav .ko-crumb a,
        #ko-nav .ko-crumb span {
          font-size: 13px; color: var(--ko-gray); text-decoration: none;
        ...
        #ko-nav .ko-sep { font-size: 12px; color: var(--ko-border); margin: 0 2px; flex-shrink: 0; }
        #ko-nav .ko-user { display: flex; align-items: center; gap: 10px; flex-shrink: 0; margin-left: auto; }
        #ko-nav .ko-user-name { font-size: 12px; color: var(--ko-gray); white-space: nowrap; max-width: 120px; overflow: hidden; text-overflow: ellipsis; }
        #ko-nav .ko-avatar { width: 28px; height: 28px; border-radius: 50%; background: var(--ko-black); color: var(--ko-cream); display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 600; flex-shrink: 0; cursor: pointer; }
        #ko-nav .ko-logout { font-size: 12px; color: var(--ko-gray); cursor: pointer; padding: 5px 8px; border-radius: 6px; border: 0.5px solid var(--ko-border); background: none; font-family: inherit; transition: background 0.15s; white-space: nowrap; }
        ...
        @media (max-width: 480px) {
          ...
          #ko-nav .ko-crumb a, #ko-nav .ko-crumb span { max-width: 100px; }
        }
      `;
